...
const ListArtists = () => {
  ...
  const fetchArtistData = async () => {
    try {
      ...
      setArtists(data || []); // Adjust based on actual response structure
    } catch (err) {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    if (formData.name === editArtist.name && formData.bio === editArtist.bio && !formData.avatar) {
    ...
    try {
      ...
      const response = await fetch('https://ns59azyddc.execute-api.eu-north-1.amazonaws.com/prod/updateartist', {
        method: "PUT",
  ...
  const handleDelete = async (artistId) => {
    try {
      const response = await fetch(`https://ns59azyddc.execute-api.eu-north-1.amazonaws.com/prod/deleteartist?artistId=${artistId}`, {
        method: 'DELETE'
      });

      if (!response.ok) throw new Error('Artist Already Available in Albums');

      const result = await response.json();
      fetchArtistData(); // Refresh artist list after deletion
      toast.success(result.message || "Artist deleted successfully!");
    } catch (error) {
